#ifndef ECPROJECT_UPLRC_CLASS_ALGORITHM_H
#define ECPROJECT_UPLRC_CLASS_ALGORITHM_H
...
namespace ECProject
{
  ...
  {
    enum class UpLRCDataBlockClass
    {
      CLASS1 = 1,
      CLASS2 = 2,
      CLASS3 = 3,
    ...
    struct UpLRCIngressParityWriteRec
    {
      int block_id = -1;
    ...
    struct UpLRCIngressClusterHints
    {
      std::vector<UpLRCIngressParityWriteRec> local_parity_writes;
      std::vector<UpLRCIngressParityWriteRec> global_parity_writes;
      std::vector<int32_t> cache_lp_stripe_groups;
    };

    /** UpLRC：按 class1/2/3 生成 train_route 并调度 timeslot */
