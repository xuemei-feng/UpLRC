...
namespace ECProject
{
  ...
  private:
    ...
    /** 用随机字节填充缓冲区；每条 stripe 在 set() 时重新调用，保证不同条带数据不同。 */
    static void fill_buffer_random(char *buf, size_t buf_size)
    {
      ...
      for (; i + sizeof(uint64_t) <= buf_size; i += sizeof(uint64_t))
      {
        ...
        std::memcpy(buf + i, &r, sizeof(r));
      }
      for (; i < buf_size; ++i)
      {
        buf[i] = static_cast<char>(gen() & 0xFF);
      }
    }

    void fill_pre_allocated_buffer_random()
    {
      const size_t buf_size = static_cast<size_t>(m_sys_config->BlockSize) * static_cast<size_t>(m_sys_config->n);
      fill_buffer_random(m_pre_allocated_buffer, buf_size);
    }
  };
