...
namespace ECProject
{
  namespace uplrc_alg2
  {
        ...
        const TransferParams &tp)
    {
      ...
      int gi = 0;
      std::cout << "[UpLRC-Alg2] ===== build_algorithm2 start: |U|=" << U.size() << " r=" << r
      ...
      for (const std::vector<int> &N : U)
      {
        ...
        std::cout << "[UpLRC-Alg2] --- group gi=" << gi << " |N|=" << N.size() << " blocks=[";
        ...
        if (static_cast<int>(N.size()) > 1)
        {
          ...
          {
            std::cout << "[UpLRC-Alg2]   skip: parity_global_b=0\n";
            ++gi;
            continue;
          }
          ...
          std::cout << "[UpLRC-Alg2]   parity_global_hull=" << parity_global_b
          ...
          auto emit_star_from_center = [&](int best_c, int cc_center) {
            std::cout << "[UpLRC-Alg2]   >>> STAR center=global_blk" << best_c << " cluster=c" << cc_center << "\n";
            for (int d : N)
            {
              ...
              L.delta_kind = UpLRCDeltaPayloadKind::DATA_DELTA;
              std::cout << "[UpLRC-Alg2]     DATA_TO_CENTER: blk" << d << "(c" << dc << ") --ΔD "
            ...
            for (int g = k; g < k + r; ++g)
            {
              ...
              L.delta_kind = UpLRCDeltaPayloadKind::PARITY_DELTA;
              L.parity_merge_data_block_ids.assign(N.begin(), N.end());
              std::cout << "[UpLRC-Alg2]     CTR_TO_GLOBAL:  global_blk" << best_c << "(c" << cc_center
            ...
            for (int gnum : groups_touched)
            {
              ...
              std::cout << "[UpLRC-Alg2]     CTR_TO_LOCAL:   global_blk" << best_c << "(c" << cc_center
          ...
          {
            std::cout << "[UpLRC-Alg2]   |N|=" << N.size() << " >= 3, attempting Algorithm3 PDP+DCP fusion...\n";
            uplrc_alg3::Algorithm3Result alg3 =
                uplrc_alg3::build_algorithm3(stripe, block_intervals, N, cluster_num, tp);
            if (alg3.applied && alg3.G.size() == alg3.dcp.group_to_collector.size() && !alg3.G.empty())
            {
              ...
              {
                fused_alg3 = true;
                std::cout << "[UpLRC] Algorithm 2+3 fused |N|=" << N.size() << " P=" << alg3.G.size()
                ...
                for (size_t j = 0; j < alg3.G.size(); ++j) {
                  ...
                  int cidx = alg3.dcp.group_to_collector[j];
                  std::cout << "[UpLRC-Alg2]   PDP group[" << j << "] → collector=global_blk" << (k + cidx)
                ...
                for (size_t j = 0; j < alg3.G.size(); ++j)
                {
                  ...
                  std::cout << "[UpLRC-Alg2]   >>> PDP group[" << j << "] → collector=global_blk" << col << " (c" << cc << ")\n";
                ...
                for (int col : used_collectors)
                {
                  ...
                  if (parity_at_col <= 0)
                    continue;
                  std::cout << "[UpLRC-Alg2]   >>> collector=global_blk" << col << " (c" << cc
                  ...
                  for (int gpar = k; gpar < k + r; ++gpar)
                  {
                    ...
                    std::cout << "[UpLRC-Alg2]     CTR_TO_GLOBAL:  global_blk" << col << "(c" << cc
                  ...
                  for (int gnum : groups_touched)
                  {
                    ...
                    std::cout << "[UpLRC-Alg2]     CTR_TO_LOCAL:   global_blk" << col << "(c" << cc
          ...
          {
            if (static_cast<int>(N.size()) >= 3)
              std::cout << "[UpLRC-Alg2]   Algorithm3 not applied, fallback to single-center STAR\n";
            else
              std::cout << "[UpLRC-Alg2]   |N|=2, using single-center STAR\n";
            int best_c = k;
            ...
            for (int cand = k; cand < k + r; ++cand)
            {
              ...
              std::cout << "[UpLRC-Alg2]   candidate center=global_blk" << cand << "(c" << cc
                        << ") sum_cost=" << sum << "s\n";
              if (sum < best_cost)
              {
                best_cost = sum;
            ...
            std::cout << "[UpLRC-Alg2]   chosen center=global_blk" << best_c << " c" << best_cc
                      << " cost=" << best_cost << "s\n";
        ...
        else
        {
          ...
          if (bd <= 0)
          {
            std::cout << "[UpLRC-Alg2]   skip MST: ΔD=0 for blk" << d << "\n";
            ++gi;
            continue;
          }
          ...
          std::cout << "[UpLRC-Alg2]   MST |N|=1 data_blk" << d << "(c" << dc_src
                    << ") ΔD=" << bd << "B  V={blk" << d;
          for (int j = 0; j < r; ++j)
          ...
          std::cout << "[UpLRC-Alg2]   MST Kruskal edges selected:\n";
          for (const auto &e : edges)
          {
            if (dsu.unite(e.u, e.v)) {
              mst.push_back({e.u, e.v});
              std::cout << "[UpLRC-Alg2]     " << vid[e.u] << " -- " << vid[e.v]
                        << " (idx " << e.u << "-" << e.v << ") w=" << e.w << "s\n";
            }
          }
          std::vector<std::pair<int, int>> directed;
          orient_mst_from_root(mst, 0, numV, &directed);
          std::cout << "[UpLRC-Alg2]   MST directed (root=blk" << d << "):\n";
          for (auto pr : directed)
          {
            ...
            std::cout << "[UpLRC-Alg2]     MST_FORWARD: blk" << sb << "(c" << sc << ") --ΔD "
                      << bd << "B --> blk" << db << "(c" << dc
                      << ") est=" << L.est_transfer_sec << "s\n";
            out.train_route.push_back(std::move(L));
          }
      ...
      std::cout << "[UpLRC-Alg2] ===== Transfer scheduling: " << out.train_route.size()
      ...
      for (size_t li = 0; li < out.train_route.size(); ++li) {
        ...
        std::cout << "[UpLRC-Alg2]   link[" << li << "] " << train_link_kind_name(L.kind)
                  << " blk" << L.src_block_id << "(c" << L.src_cluster << ")->blk" << L.dst_block_id
                  << "(c" << L.dst_cluster << ") payload=" << L.payload_bytes
                  << "B grp=" << L.group_index;
        if (L.delta_kind == UpLRCDeltaPayloadKind::PARITY_DELTA)
      ...
      while (true)
      {
        ...
        // 打印本时间步候选链路
        std::cout << "[UpLRC-Alg2] --- step " << ts << " candidates (pending & eligible):\n";
        for (size_t i = 0; i < out.train_route.size(); ++i)
        {
          ...
          const TrainLink &L = out.train_route[i];
          std::cout << "[UpLRC-Alg2]   link[" << i << "] blk" << L.src_block_id << "->blk" << L.dst_block_id
        ...
        if (used.empty())
        {
          ...
          for (size_t i = 0; i < remaining.size(); ++i)
          {
            ...
            {
              std::cout << "[UpLRC-Alg2]   [deadlock-prevention] forcing link[" << i << "]\n";
              used.push_back(static_cast<int>(i));
              break;
            }
          }
        }
        std::cout << "[UpLRC-Alg2]   step " << ts << " selected " << used.size() << " link(s): [";
      ...
      std::cout << "[UpLRC-Alg2] ===== Transfer scheduling done: total_steps=" << ts << " =====\n";
      return out;
    }
    ...
    bool load_bw_matrix_from_limitsame_file(const std::string &path, int cluster_num, TransferParams *tp)
    {
      ...
      {
        std::cout << "[UpLRC-Class] BW matrix parse failed: need " << (cluster_num * cluster_num) << " got "
                  << vals.size() << " from " << path << "\n";
        return false;
      }
      ...
      if (uplrc_verbose_enabled())
        std::cout << "[UpLRC-Class] Loaded BW matrix " << cluster_num << "x" << cluster_num << " from " << path << "\n";
      return true;
    }
  } // namespace uplrc_alg2
} // namespace ECProject
